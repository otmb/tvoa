...
// in simulator we redefine to format for tvoa Developer console
...
{\
	NSString * exceptionName = [@"org.tvoa." stringByAppendingString:NSStringFromClass([self class])];\
...
+ (void) throwException:(NSString *) reason subreason:(NSString*)subreason location:(NSString *)location\
{\
	NSString * exceptionName = @"org.tvoa";\
	NSString * message = [NSString stringWithFormat:@"%@. %@ %@",reason,(subreason!=nil?subreason:@""),(location!=nil?location:@"")];\
	TiExceptionThrowWithNameAndReason(exceptionName,message);\
}\
...
enum {
	UItvoaNativeItemNone = -1, 
	UItvoaNativeItemSpinner = -2,
	UItvoaNativeItemProgressBar = -3,
	
	UItvoaNativeItemSlider = -4,
	UItvoaNativeItemSwitch = -5,
	UItvoaNativeItemMultiButton = -6,
	UItvoaNativeItemSegmented = -7,
	
	UItvoaNativeItemTextView = -8,
	UItvoaNativeItemTextField = -9,
	UItvoaNativeItemSearchBar = -10,
	
	UItvoaNativeItemPicker = -11,
	UItvoaNativeItemDatePicker = -12,
	
	UItvoaNativeItemInfoLight = -13,
	UItvoaNativeItemInfoDark = -14,
	
	UItvoaNativeItemDisclosure = -15,
	
	UItvoaNativeItemContactAdd = -16
};
...
/**
 *	TiThreadPerformOnMainThread should replace all tvoa instances of
